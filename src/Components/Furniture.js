import React, { useEffect, useState } from 'react'
import darkImage from '../images/image-about-dark.jpg'
import lightImage from '../images/image-about-light.jpg'
import { getUsers } from './Request'

const Furniture = () => {
    const [data, setData] = useState([])

    useEffect(() => {
        getUsers().then(data =>{
            setData(getUsers)
            console.log(data)
        })
    }, [])
  return (
    <>
        <section className='grid grid-cols-1 lg:grid-cols-3'>
            <article>
                <img src={darkImage} alt="dark-image" className='w-full' />
            </article>
            <article className='p-8 lg:p-12'>
                <h1 className='font-semibold text-2xl lg:text-3xl uppercase'>About our furniture</h1>
                <p className='text-slate-900 opacity-75'>
                    Our multifunctional collection blends design and function to suit your individual taste.
                    Make each room unique, or pick a cohesive theme that best express your interests and what
                    inspires you. Find the furniture pieces you need, from traditional to contemporary styles
                    or anything in between. Product specialists are available to help you create your dream space.
                </p>
            </article>
            <article>
                <img src={lightImage} alt="light-image" className='w-full' />
            </article>
            
        </section>
        <section>
            {
                data.map((items, index) => (
                    <div className='' key={index}>
                        <p>{items.email}</p>
                    </div>
                ))
            }
        </section>
    </>
  )
}

export default Furniture